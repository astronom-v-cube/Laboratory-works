...
v = np.loadtxt("LW210-4.txt")       # открыть файл с данными
...
freq_teor = np.linspace(freq[0], freq[-1], 2048)
w = freq*2*np.pi
w_teor = freq_teor*2*np.pi

teor = np.sqrt(((2*w_teor**2*L**2*R+R**3)/(w_teor**2*L**2+R**2))**2+((w_teor*L*R**2)/(w_teor**2*L**2+R**2))**2)


fig = plt.figure() 					# тело графика
plt.title ("Импеданс схемы №4")	    # название графика
...
# plt.plot(nu, z, label='Название линии')        # стандартный график (ломаная)
