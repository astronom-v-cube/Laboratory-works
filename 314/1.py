...
v = np.loadtxt("LW314-1.txt")       # открыть файл с данными
...
plt.title ("График квадрата диаметра кольца от его порядового номера")	    # название графика
plt.xlabel("m", fontsize=10, color='blue')		# ось абсцисс (x)
plt.ylabel(r'$d_{m}^2$', fontsize=10, color='red') 		# ось ординат (y)
# plt.grid(True)                                # включение отображения стандартой сетки
# plt.plot(x, y, label='Название линии')        # стандартный график (ломаная)
plt.plot(x, ya, label='График аппроксимации')        # график - апрокимация (прямая)
...
plt.scatter(x, y, s=10, c='red', marker="o", alpha = 1, label='Усредненные результаты измерений')     # отображение точек
