...
import numpy as np
Это редактор кода, просто штука которая сохраняет написанный код
v = np.loadtxt("LW222-5.txt")       # открыть файл с данными
y = [item[0] for item in v]			# первый столбец - y
x = [item[1] for item in v]			# второй стобец - x
# p = np.polyfit(x, y, 3)				# создание полинома первой степени - апроксимация
# ya = np.polyval(p, x)				# координаты y для полинома

fig = plt.figure() 					# тело графика
plt.title ("T = f(Ɛ)")	# название графика
plt.xlabel("Ɛ, В", fontsize=10, color='blue')		# ось абсцисс (x)
plt.ylabel("T, мс", fontsize=10, color='red') 		# ось ординат (y)
plt.grid(True)      # включение отображения стандартой сетки
# plt.plot(x, y, label='T = f(Ɛ)')        # стандартный график (ломаная)
# plt.plot(x, ya, label='T = f(R)')        # график - апрокимация (прямая)
...
plt.scatter(x, y, s=7, c='red', marker="o", alpha = 0.9, label='Результаты измерений')     # отображение точек
