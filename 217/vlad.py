...
v = np.loadtxt("vlad.txt")       # открыть файл с данными
...
plt.title ("Исследование вынужденных колебаний в электрическом контуре")	    # название графика
plt.xlabel("Частота (Гц)", fontsize=10, color='blue')		# ось абсцисс (x)
plt.ylabel("Напряжение (В)", fontsize=10, color='red') 		# ось ординат (y)
# plt.grid(True)                                # включение отображения стандартой сетки
plt.plot(nu, Urf, c='blue', label=r'Теоретический график $U_{R}$')        # стандартный график (ломаная)
plt.plot(nu, Ucf, c='red', label=r'Теоретический график $U_{С}$')        # стандартный график (ломаная)
plt.plot(nu, Ulf, c='green', label=r'Теоретический график $U_{L}$')        # стандартный график (ломаная)
...
plt.scatter(y, x1, s=10, c='red', marker="o", alpha = 1, label=r'Практические значения $U_{С}$')     # отображение точек
plt.scatter(y, x2, s=10, c='green', marker="x", alpha = 1, label=r'Практические значения $U_{L}$')     # отображение точек
plt.scatter(y, x3, s=10, c='blue', marker="D", alpha = 1, label=r'Практические значения $U_{R}$')     # отображение точек
