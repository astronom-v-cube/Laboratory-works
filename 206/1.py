...
import numpy as np
n = np.arange(1,11)
v = np.loadtxt("LW206-1.txt")       # открыть файл с данными
v[:, 0] = 90
An = [item[0] for item in v]			# первый столбец - y
# absan = [item[1] for item in v]			# второй стобец - x
...
plt.title ("Меандр. Фазы")	    # название графика
plt.xlabel("ω", fontsize=10, color='blue')		# ось абсцисс (x)
plt.ylabel("θ", fontsize=10, color='red') 		# ось ординат (y)
...
plt.bar(n, An, width = 0.4)
plt.scatter(n, An, s=15, c='red', marker="o", alpha = 1, label='Теоретические значения')     # отображение точек
